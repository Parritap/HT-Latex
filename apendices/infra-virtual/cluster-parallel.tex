...
# Debemos estar dentro de la carpeta que contiene el archibo .tar de openmpi
...
\FloatBarrier\subsection{Configurción de OpenMPI en Condor}

Como primer paso, es necesario crear un nuevo archivo de configuración en el que se especifique cuál será el Scheduler dedicado para los nodos ejecutores. Además, deben definirse ciertos parámetros para que estos nodos prioricen los trabajos dedicados y se les indique que no suspendan los trabajos enviados. La documentación oficial de HTCondor \cite{HTCondor_Parallel} recomienda reservar un conjunto de máquinas exclusivas para el universo parallel. En el caso de este clúster, se optó por realizar esta configuración en el archivo \texttt{/etc/condor/config.d/50-parallel.config} en cada uno de los nodos ejecutores, cuyo contenido se muestra a continuación:
